...
{
   public static class CodeWars_SetTwo
    {               
     ...
     "Lima:Jan 1.2,Feb 0.9,Mar 0.7,Apr 0.4,May 0.6,Jun 1.8,Jul 4.4,Aug 3.1,Sep 3.3,Oct 1.7,Nov 0.5,Dec 0.7";
        ...
        public static double Variance(string town, string data2)
        {
            double avg = -1,variance=-1;
            ...
            if (moreString.Count > 0)
            {
                ...
                avg = values.Select(x => double.Parse(x)).Average();
                variance = values.Select(x => double.Parse(x)).Select(x => Math.Pow(x - avg, 2)).Average();
            }
            return variance;
        }
